...
import { BiHeart, BiJoystick } from "react-icons/bi";
...
import '../App.css'
const Saidbar = () => {
  const { togle, counter } = useContext(MyContext);
  ...
  return (
    <>
      {!togle ? (
        <div className={`minu ${counter ? 'blackMode' : ''}`} >
        ...
        <div className={`bar ${counter ? 'blackMode' : ''}`}>
          ...
          <NavLink to='/watch' className='bar__link flex items-center'>
            <BsStopwatch className='icons' />
            <span className='bar__text block w-24'>Watch Later</span>
          </NavLink>
          ...
          {!open ? (
            ...
            <div  className={`sub absolute top-64 p-3  rounded ${counter ? 'blackModeeee' : 'lightMode'}`}>
      ...
      <div className=' '>
        <div className={`minu1 py-5   fixed px-3  ${counter ? 'blackMode' : ''}`}>
          <NavLink to='/' className=''>
            <AiOutlineHome className='icons1' />
          </NavLink>
          <NavLink to='shorts' className=''>
